...
byte *buildFrame_i(int fd, size_t *size) {
  ...
  while (reading) {
    ...
    printf("Frame Before logic : ");printHexN(frameI,currentByte);printf("\n");
...
byte *byteStuff(const byte *array, size_t *size) {
  ...
  size_t array_it = 0;
  while (array_it < *size) {
    if (array[array_it] == FLAG || array[array_it] == ESCAPE_CHR) stuffedSize++;
    stuffedSize++;
    array_it++;
  }
...
byte *byteDeStuff(const byte *array, size_t *size) {
  ...
  while (array_it < *size) {
    deStuffedSize++;
    if (array[array_it] == ESCAPE_CHR) deStuffedSize--;
    array_it++;
  }
...
byte *bufferToFrameI(const byte *buf, size_t *size, int number) {
  ...
  *size = toStuffSize;
  ...
  for (size_t c = 0; c < *size; c++) finalFrame[f_it++] = stuffedFrame[c];
